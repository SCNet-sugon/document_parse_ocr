...
description: 支持文档智能识别（异步），适用于大批量文档处理场景。提交公网可访问的文件 URL，自动识别文档中的文本、表格、标题等结构信息，返回结构化的 JSON 结果文件下载地址。**请确保您拥有处理该文档的合法权利，且文档内容不违反任何法律法规。**
version: 1.0.0
author: SCNet
license: MIT
tags:
...
- 本技能仅将用户提供的 `fileUrl` 转发至 Scnet API 进行文档识别，**不存储、不缓存**任何文件内容。
- 识别结果由 Scnet API 生成，结果文件下载地址有效期为 12 小时，请用户及时获取并自行保管。
- 用户应确保所提交文件的内容不违反法律法规，不侵犯他人合法权益，并已获得必要的授权。
- 使用本技能即表示您同意将文件内容发送至第三方服务商（Scnet）进行处理，具体数据处理政策请参考 [Scnet 官网](https://www.scnet.cn)。
